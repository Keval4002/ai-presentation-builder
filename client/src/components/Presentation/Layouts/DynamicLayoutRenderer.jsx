...
function DynamicLayoutRenderer({ slide, theme, layout, slideIndex, onLayoutMeasure, onScaleReport }) {
    ...
    const renderTitle = () => {
        ...
        return (
            <div ref={(el) => createRef('title', el)} style={{ 
                ...toCss(positions.title), 
                display: 'flex', 
                alignItems: isTitleOnly ? 'center' : 'flex-start', 
                justifyContent: 'center', 
                padding: '0 0.5rem' 
            }}>
                <h3 style={{ 
                    fontFamily: unifiedTheme.headingFont, 
    ...
    const renderImage = () => {
        ...
        return (
            <div ref={(el) => createRef('image', el)} style={{ 
                ...toCss(positions.image), 
                display: 'flex', 
                alignItems: 'center', 
                justifyContent: 'center', 
                overflow: 'hidden'
    ...
    const renderContentBlock = (items, key) => (
        <div ref={(el) => createRef(key, el)} style={{ 
            ...toCss(positions[key]), 
            overflow: 'hidden', 
            padding: '0.5rem' 
        }}>
            <AdaptiveContentContainer 
                onMeasure={onScaleReport} 
    ...
    const renderTimelineItems = () => {
        if (!layoutName.includes('timeline')) return null;
        return parsedContent.items.map((item, i) => (
            <React.Fragment key={`timeline-item-${i}`}>
                <div ref={(el) => createRef(`item${i}C`, el)} style={{ 
                    ...toCss(positions[`item${i}C`]), 
                    display: 'flex', 
                    alignItems: 'center', 
                    justifyContent: 'center', 
                    background: unifiedTheme.primary, 
                    color: unifiedTheme.background, 
                    borderRadius: '50%', 
                    zIndex: 2, 
                    fontSize: `${FIXED_FONT_SIZES.CONTENT_TIMELINE}px`, 
                    fontWeight: 'bold' 
                }}>
                    {i + 1}
                </div>
                <div ref={(el) => createRef(`item${i}T`, el)} style={{ 
                    ...toCss(positions[`item${i}T`]), 
                    display: 'flex', 
                    alignItems: 'center', 
                    zIndex: 1, 
                    padding: '0.5rem' 
